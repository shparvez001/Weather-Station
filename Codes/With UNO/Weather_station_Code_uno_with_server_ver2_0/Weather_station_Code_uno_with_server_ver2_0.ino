...
//LCD COde for sending data to server
...

{
    ...
    lcd.print("Sending temperature data to server");
...
/*
    // SMS sending portion of the code
    if (m%10==0)
    {
        lcd.clear();
        lcd.setCursor(0,0);
...
void init_temp( )
{
    ...
    {
        samples[i] = ( 4.5 * analogRead(tempPin) * 100.0) / 1024.0; // conversion math of LM35 sample to readable temperature and stores result to samples array. 1024 is the Bit depth (quantization) of Arduino.
